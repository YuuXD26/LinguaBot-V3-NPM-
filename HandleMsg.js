...
let settingapi = require("./key.json");
...
const notice = ['Lingua Bot', 'Lingua Bot']
...
module.exports = HandleMsg = async (piyo, message) => {
        ...
        const cecan = [
            {
                ...
                imagex: "https://i.ibb.co/Jr5SdmG/michie3.jpg",
            },
            {
                lahwoi: "Pacar piyo yang ke 2",
                imagex: "https://i.ibb.co/60YHk5M/michie2.jpg",
            },
            {
                lahwoi: "Pacar piyo yang ke 3",
                imagex: "https://i.ibb.co/whsRZJ8/michie.jpg",
            },
            {
                lahwoi: "Pacar piyo yang ke 4",
                imagex: "https://i.postimg.cc/W3F9MmJR/Michiejkt48-mirror-selca.jpg",
            },
            {
                lahwoi: "Pacar piyo yang ke 5",
                imagex: "https://i.postimg.cc/28sB6QCb/michie-JKT48.jpg",
            },
            {
                lahwoi: "Pacar piyo yang ke 6",
                imagex: "https://i.postimg.cc/0QkSv9PV/Michie-jkt48-1.jpg",
            },
            {
                lahwoi: "Pacar piyo yang ke 7",
                imagex: "https://i.postimg.cc/Jnpk4ShM/Michie-JKT48-2.jpg",
            },
            {
                lahwoi: "Pacar piyo yang ke 8",
                imagex: "https://i.postimg.cc/7hbTCXC7/michie-jkt48-3.jpg",
            },
            {
                lahwoi: "Pacar piyo yang ke terakhir",
                imagex: "https://i.postimg.cc/FH0kJYKP/Michie-jkt48-Spider-girl.jpg"
            }
        ]
        const santet = [
            'Muntah Paku',
        ...
        function isStickerMsg(id) {
            ...
            for (let i of stickerspam) {
                if (i.id === id) {
                    if (i.msg >= 10) {
                        found === true
                        piyo.reply(from, '*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\nKamu telah SPAM STICKER di grup, kamu akan di kick otomatis oleh Lingua Bot', message.id).then(() => {
        ...
        if (!gg && !hh) {
            switch (command) {
                ...
                case 'menu':
                    ...
                    let tod = `${timee}`;
                    await piyo.sendText(from, menuId.textmenubaru(pushname, premi, _registered, coloo, tod))
                    break
                case 'rules':
                case 'rule':
                ...
                case 'owner':
                    ...
                    break
                case 'bal':
                    if (!isRegistered) return piyo.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lingua Bot, untuk pendaftaran bisa menggunakan /register nama | Jenis Kelamin. Contoh: /register ${pushname}|cewe`, id)
                    ...
                    case "ai": case "openai":
  try {
    if (settingapi.keyopenai === "null") return reply("Apikey belum diisi\n\nSilahkan isi terlebih dahulu apikeynya di file key.json\n\nApikeynya bisa dibuat di website: https://beta.openai.com/account/api-keys");
    if (!text) return reply(`Chat dengan AI.\n\nContoh:\n${prefix}${command} Apa itu resesi`);
    const configuration = new Configuration({
      apiKey: settingapi.keyopenai,
    });
    ...
    if (response.data && response.data.choices && response.data.choices[0] && response.data.choices[0].text) {
      piyo.reply(`${response.data.choices[0].text}`);
    } else {
        piyo.reply("Maaf, tidak ada hasil yang ditemukan.");
    }
  } 
  catch (error) {
    if (error.response) {
      ...
      console.log(error);
      piyo.reply("Maaf, sepertinya ada yang error :"+ error.message);
    }
  }
  break;
  ...
  case 'cekgay':
    if (args.length === 0) {
        piyo.reply(`Format penggunaan salah. Contoh penggunaan: ${prefix}cekgay (nama)`);
    } else {
      ...
      if (filtered.length > 0) {
        piyo.reply(`Maaf ya sayang, ${text} bukan orang gay !`);
      } else {
...
case "cinta":
  if (args.length < 3 || (!args.includes("dan") && !args.includes("dengan"))) {
    return piyo.reply(`Format yang benar: ${prefix}cekcinta (nama kamu) dan (nama pasangan)`);
  }
  ...
  case "kapan":
    if (args.length === 0) {
      return piyo.reply(`Format yang benar: ${prefix}kapan {pertanyaan}`);
    }
    ...
    case "bisakah":
      ...
      piyo.reply(hasilJawaban3);
      break;
      case "apakah":
        if (args.length === 0) {
          return piyo.reply(`Format yang benar: ${prefix}apakah {pertanyaan}`);
        }
      ...
      break;
                ...
                case 'getstiker':
                    ...
                    try {
                        ...
                        await piyo.sendImageAsSticker(from, imageBase64, { author: 'Piyo', pack: 'Lingua Bot' })
                    } catch (err) {
                ...
                //     break
                // case 'pornhub':
                //     if (args.length == 2) return piyo.reply(from, `Kirim perintah */pornhub [ |Teks1|Teks2 ]*, contoh */pornhub |Alvio|Lingua Bot*`, id)
                    ...
                    if (isOwnerBot) {
                        await piyo.joinGroupViaLink(linkgrup)
                            .then(async () => {
                                ...
                                await piyo.sendText(chekgrup.id, `Halo Anjg~, Im Lingua Bot. Untuk Memulai Bot silahkan ketik  ${prefix}menu`)
                            })
                    } else {
                ...
                case 'next':
                    ...
                    break
                case 'send':
                    if (!isRegistered) return piyo.reply(from, `Maaf ${pushname}, sepertinya kamu belum terdaftar sebagai user Lingua Bot, untuk pendaftaran bisa menggunakan */register* nama | no hp.    Contoh: /register |${pushname}|cewe/cowo`, id)
                ...
                case 'nuliskanan': {
                    ...
                    spawn('convert', [
                        ...
                        .on('exit', () => {
                            piyo.sendImage(from, './media/images/buku/setelahkanan.jpg', 'after.jpg', `Nih Hasilnya \nDitulis selama: ${processTime(t, moment())} _detik_`, id)
                        })
                }
                    break
                case 'nuliskiri':
                    ...
                    spawn('convert', [
                        ...
                        .on('error', () => piyo.reply(from, 'Error gan', id))
                        .on('exit', () => {
                            piyo.sendImage(from, './media/images/buku/setelahkiri.jpg', 'after.jpg', `Nih hasilnya \nDitulis selama: ${processTime(t, moment())} _detik_`, id)
                        })
                    break
                case 'foliokiri': {
                    //if (!isPremium) return piyo.reply(from, 'Maaf, ini adalah fitur premium, untuk menggunakan fitur ini silahkan donasi, Kirim #donasi untuk melihat info donasi', id)
...
//                     break
                ...
                //     if (!isOwnerBot) return piyo.reply(from, `Fitur Ini Khusus Premium `, id)
                ...
                //     break
                // case 'waktu':
                //     piyo.reply(from, `*Waktu Lingua Bot Aktif* \n\nPAGI = 07:00 - 10:00\n\nSIANG = 13:00 - 16:00\n\nMALAM = 19:00 - 22:00`, id)
                //     break
                // case 'listbacot':
                //     const bacul = dbcot
                ...
                case 'lingua':
                    if (isGroupMsg) {
                        ...
                        if (args[0] == 'on') {
                            ...
                            chatt.splice(inxx, 1)
                            fs.writeFileSync('./settings/piyo.json', JSON.stringify(chatt))
                            piyo.reply(from, 'Menonaktifkan bot lingua!', id)
                        } else {
                            piyo.reply(from, `Untuk mengaktifkan lingua \n\nPenggunaan\n${prefix}lingua on --mengaktifkan\n${prefix}lingua off --nonaktifkan\n`, id)
                        }
                    }
                    break
                // case 'simisimi':
                ...
                //////////////////////////////////////////////////////Owner Bot////////////////////////////////////////////////////
                case 'getses':
                    if (!isOwnerBot) return piyo.reply(dari, 'Perintah ini hanya untuk Owner lingua', id)
                ...
                case 'bcimg':
                    ...
                    if (isMedia && type === 'image' || isQuotedImage) {
                        ...
                        for (let grp of chaim) {
                            ...
                            if (!cukk.isReadOnly) piyo.sendFile(grp, `./media/images/bc.jpg`, 'piyo.jpg', `[Lingua Bot BROADCAST]\n\n${q}`, '', id)
                            if (cukk.isReadOnly) piyo.sendFile(grp, `./media/images/bc.jpg`, 'piyo.jpg', `[Lingua Bot BROADCAST]\n\n${q}`, '', id)
                        }
            ...
            if ((!isCmd && chatId && isChat) && message.type === 'chat') {
                ...
                await piyo.reply(from, `Lingua Bot Menjawab: ${piyoget.data.result}`, id)
            }
            // Simi-simi function
